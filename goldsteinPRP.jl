# function goldsteinPRP(x, f, ∇f, n; maxk, ϵ)
#-----------------------------------------------
# function goldsteinPRP implements the PRP method (conjugate gradient) or the gradient method where the steplength holds the Goldstein conditions.

#
# Reference: Ribeiro, A. A., & Karas, E. W. (2013). Otimização Contínua: aspectos teóricos e computacionais.
...
function goldsteinPRP(x, f, ∇f, n; maxk = 1000, ϵ = 1e-5, method = 1)
    ...
    while norm(∇f(x)) > ϵ && k < maxk                   # stop conditions
        ...
        t, error, fn_int = goldstein(f, ∇f, x, d)
        fn += fn_int
    ...
    if k < maxk
        ...
        normx = norm(gradfx)
        et = time() - t
        return x, fx, normx, k, et, ierror, serror, counter, fn, X, Y, Z
    else
